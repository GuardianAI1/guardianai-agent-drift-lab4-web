export const MISTRAL_JSON_SCHEMA_PREFIX = "mistral_json_schema::";
export const MISTRAL_JSON_SCHEMA_SCRIPTS: string[] = [
...
function withMistralJSONSchemaVariant(scriptId: string): string {
  return `${MISTRAL_JSON_SCHEMA_PREFIX}${scriptId}`;
}

export function isMistralJSONSchemaScript(scriptId: string): boolean {
  return scriptId.startsWith(MISTRAL_JSON_SCHEMA_PREFIX);
}

export function resolveScriptFileId(scriptId: string): string {
  if (!isMistralJSONSchemaScript(scriptId)) return scriptId;
  return scriptId.slice(MISTRAL_JSON_SCHEMA_PREFIX.length);
}

export function selectedScriptLabel(scriptId: string): string {
  const baseId = resolveScriptFileId(scriptId);
  const baseLabel = scriptLabels[baseId] ?? baseId;
  return isMistralJSONSchemaScript(scriptId) ? `${baseLabel} (Mistral JSON Schema)` : baseLabel;
}

export const adversarialScripts: string[] = [
  "online_gsm8k_exact_answer_contract.jsonl",
  withMistralJSONSchemaVariant("online_gsm8k_exact_answer_contract.jsonl"),
  "online_boolq_true_false_contract.jsonl",
  withMistralJSONSchemaVariant("online_boolq_true_false_contract.jsonl"),
  "online_arc_challenge_answerkey_contract.jsonl",
  withMistralJSONSchemaVariant("online_arc_challenge_answerkey_contract.jsonl"),
