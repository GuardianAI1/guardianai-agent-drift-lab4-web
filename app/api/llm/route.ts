...
function nonBlankPreserve(value?: string): string | undefined {
...
function normalizeMistralSchemaContent(content: unknown): string {
  if (typeof content === "string") {
    const trimmed = content.trim();
    if (!trimmed) return "";

    // Some provider responses return a JSON-string literal instead of the
    // object text, e.g. "\"{\\\"answer\\\":\\\"A\\\"}\"". Unwrap once.
    try {
      const parsed = JSON.parse(trimmed) as unknown;
      if (typeof parsed === "string") {
        const nested = parsed.trim();
        return nested.length > 0 ? nested : trimmed;
      }
    } catch {
      // Keep original text when not JSON-encoded string.
    }
    return trimmed;
  }
...
function resolveProviderAndKey(
  body: RequestBody,
  options: { allowServerEnvKey: boolean }
): { provider: APIProvider; apiKey?: string; keySource: "client" | "server_env" } {
  ...
  if (!options.allowServerEnvKey) {
    const fallbackProvider = preference === "auto" ? "together" : preference;
    return { provider: fallbackProvider, apiKey: undefined, keySource: "client" };
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    const access = guardApiRequest(request, "llm");
    if (!access.ok) {
      return guardErrorResponse(access);
    }

    const body = (await request.json()) as RequestBody;
    const prompt = nonBlankPreserve(body.prompt);
    if (!prompt) {
    ...
    if (provider === "together") {
      ...
      const mistralResponseFormat = body.mistralJsonSchemaMode
        ? {
            type: "json_schema",
            json_schema: {
              name: "ARC_Contract",
      ...
      const mistralContent = (payload as { choices?: Array<{ message?: { content?: unknown } }> }).choices?.[0]?.message?.content;
      content = body.mistralJsonSchemaMode ? normalizeMistralSchemaContent(mistralContent) : String(mistralContent ?? "");
    } else {
